...
class FullCudaGraphWrapper:
    ...
    def __init__(self, forward_backward_func, cuda_graph_warmup_steps=1, use_single_mempool=False, cuda_graph_data_2buffer_step=-1):
    ...
    def data_read(self, data_iterator, model, training, num_microbatches, to_cpu_cache, from_cpu_cache, to_static_gpu):
        ...
        torch.cuda.nvtx.range_push(f"cg_data_read")
        ...
        if from_cpu_cache:
            assert data_iterator is None and FullCudaGraphWrapper.cached_data_iterator[training_str] is not None
            data_iterator = FullCudaGraphWrapper.cached_data_iterator[training_str]
            FullCudaGraphWrapper.cached_data_iterator[training_str] = None
        else:
            assert data_iterator is not None

        if to_cpu_cache:
            assert FullCudaGraphWrapper.cached_data_iterator[training_str] is None
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.cuda_graph_data_2buffer_step != -1 and self.curr_iteration[training_str] >= self.cuda_graph_data_2buffer_step:
            ...
            to_static_gpu = False
            FullCudaGraphWrapper.cached_data_iterator[training_str] = self.data_read(data_iterator, model, training, num_microbatches, to_cpu_cache, from_cpu_cache, to_static_gpu)
