...
class SymmPutBuffer:
    ...
    def put_signal(self, tensor, dst, hdl):
        ...
        with torch.cuda.stream(self.stream):
            ...
            if self.old_tensor is not None:
                self.old_tensor.data = self.buffer.data # free the memory
            self.old_tensor = tensor
    ...
    def reset(self):
        ...
        if self.old_tensor is not None:
            self.old_tensor.storage().resize_(0)
            self.old_tensor = None
